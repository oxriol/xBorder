package nivult.border.task;

import nivult.border.data.BorderData;
import nivult.border.data.Config;
import nivult.border.data.CoordXZ;
import nivult.border.data.WorldFileData;
import nivult.border.event.WorldBorderFillMessageEvent;
import nivult.border.event.WorldFillerTaskCompleteEvent;
...
public class WorldFillTask implements Runnable
{
	...
	private void reportProgress()
	{
		...
		*/
		...
		sendMessage(reportNum + " more chunks processed (" + (reportTotal + reportNum) + " total, ~" + Config.coord.format(getPercentage()) + "%" + ")");
	...
	private void sendMessage(String text)
	{
        ...
        Bukkit.getPluginManager().callEvent(new WorldBorderFillMessageEvent("[Fill] " + text, getPercentage()));
	...
	private double getPercentage(){
		return Math.min(100.0, ((double) (reportTotal + reportNum) / (double) reportTarget) * 100);
	}
