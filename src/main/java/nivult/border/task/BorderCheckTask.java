...
public class BorderCheckTask implements Runnable
{
	...
	private static Location newLocation(Player player, Location loc, BorderData border, boolean notify)
	{
		...
		if (newLoc == null)
		{
			...
			int radius = WorldBorder.get().GetWorldBorder(loc.getWorld().getName()).getRadiusX();
	...
	public static void safeTeleport(final Player player, final Location location) {
		...
		if (horse instanceof Horse) {
			Horse vehicle = ((Horse) horse);
			vehicle.eject();
			// TP the horse
			Bukkit.getScheduler().runTaskLater(WorldBorder.get(),
			()-> vehicle.teleport(location.add(0, 1, 0)), 1L);

			// Reattach the player to the horse
			Bukkit.getScheduler().runTaskLater(WorldBorder.get(), ()->vehicle.addPassenger(player), 2L);
